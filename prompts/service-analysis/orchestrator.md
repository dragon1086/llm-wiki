# Service Analysis — Orchestrator Prompt

아래 전체를 복사해 Claude Code 세션에 붙여넣는다. `{SERVICE_SLUG}`만 실제 서비스명으로 치환.
...
결과는 llm-wiki vault의 raw/services/{SERVICE_SLUG}.md 로 저장될 예정.
...
- 기본: profiles/default.md 의 차원 전부 적용
...
프로파일에 명시된 순서대로 `dimensions/<name>.md` 를 각각 읽는다.
...
`dimensions/forensics.md` 는 항상 마지막에 적용. 다른 차원에서 쌓인 의문들을
...
---
type: summary
source: "service-analysis: {SERVICE_SLUG}"
source_file: "raw/services/{SERVICE_SLUG}.md"
date_ingested: "<오늘 날짜>"
profile: "<선택한 프로파일명>"
dimensions_applied: [<적용된 dimension slug들>]
topics: [microservice, spring-boot, <도메인 키워드>]
entities: [{SERVICE_SLUG}, <참조 서비스 slug들>, <DB 테이블들>]
