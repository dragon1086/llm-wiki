# cart-service 분석용 복붙 프롬프트

> **사용법**
> 1. 터미널에서 `cd /Users/kakao/workspace/cart-service && claude` 로 새 세션 시작
> 2. 아래 ``` 블록 전체 복사해서 붙여넣기
> 3. Claude가 12개 dimension 순차 적용 (domain-logic 포함 시 30~45분)
> 4. 출력 markdown을 `/Users/kakao/workspace/obsidian-vault/llm-wiki/raw/services/cart-service.md` 로 저장
> 5. llm-wiki 세션으로 돌아와서 `python scripts/wiki.py ingest raw/services/cart-service.md --debug` 로 ingest
...
결과는 llm-wiki vault의 raw/services/cart-service.md 로 저장될 예정.
...
- 관찰과 추론을 분리: 코드 근거는 `(→ CartService.kt:142)` 형식으로 인용, 추론은 `**가설:**` 로 시작.
...
---
type: summary
source: "service-analysis: cart-service"
source_file: "raw/services/cart-service.md"
date_ingested: "<오늘 날짜>"
profile: "<선택한 프로파일명>"
dimensions_applied: [<적용된 dimension slug들>]
topics: [microservice, spring-boot, <도메인 키워드>]
entities: [cart-service, <참조 서비스 slug들>, <DB 테이블들>]
...
완료되면 전체 markdown 결과를 /Users/kakao/workspace/obsidian-vault/llm-wiki/raw/services/cart-service.md 에 직접 저장하라.
```
